...
Canonical handoff doc for agents maintaining this repository — workflow, data
models, runbook, performance rules, and repo hygiene. Companion docs:

1. This file — everything needed for the daily update.
...
Visitors never download the full datasets:

- **Events:** `site/_data/events/index.json` lists five-year windows, each with a
  `days` map (MM-DD to event count). The homepage fetches only the windows that
  contain events for the displayed date and renders each as it arrives.
- **Promises:** `site/_data/promises/index.json` holds totals/status counts and a
  due-month manifest. The promises page fetches month files one at a time and
  renders each immediately. Manifest counts are a snapshot at `as_of`; the page
  selects due months and individual promises using today's UTC date and computes
  its own totals. `kept (delayed)` is included in the kept total/filter.
- Both pages keep loading incrementally until complete — no "load more" buttons.
- These files are generated by `python3 site/_data/split_data.py`; never hand-edit them.
...
1. **Events** — research `events.last_date + 1` through today UTC, ≥2 sources each,
   append, then advance `events.last_date` only through the latest fully elapsed
   UTC day researched. Today's partial research must be revisited next run.
...
model (currently `deepseek-flash`, DeepSeek-V4.1-Flash; legacy `deepseek-v4-flash`
requests are routed to it), and the daily research is the dominant cost:
...
- Commit messages use UTC timestamps (see format above); publish only when authorized.
