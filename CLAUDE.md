...
4. **Commit & push if changed:** if `events.json`, `promises.json`, `checkpoint.json`, or any generated file under `_data/events/` / `_data/promises/` changed, commit with a UTC timestamp in the message (e.g. `data: 2026-08-01T01:10:00Z — events +4, promises +4`) and push to origin (`0x8338/dontforget`, branch `main`). If a same-day re-run changes nothing, skip the commit.

## Pages loading (lightweight, no buttons)

The full datasets are never downloaded by visitors:

- **Events:** `_data/events/index.json` lists five-year windows, each with a `days` map (MM-DD → event count). The homepage fetches only the windows that contain events for the displayed date and renders each as it arrives.
- **Promises:** `_data/promises/index.json` holds totals/status counts and a due-month manifest. The promises page fetches month files one at a time and renders each immediately.
- Both pages keep loading incrementally until complete — no "load more" buttons.
- These files are generated by `python3 site/_data/split_data.py`; never hand-edit generated files.
